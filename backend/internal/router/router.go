...
func New(cfg config.Config, h *handler.Handler) *gin.Engine {
	...
	api := router.Group("/api")
	{
		...
		admin.Use(h.AuthRequired(), h.AdminRequired())
		{
			...
			admin.GET("/notifications", h.AdminNotifications)
			admin.DELETE("/notifications", h.DeleteNotifications)
			admin.POST("/notifications/clear", h.DeleteNotifications)
			admin.POST("/notifications/delete", h.DeleteNotifications)
			admin.PATCH("/notifications/read", h.MarkNotificationsRead)
